    ...
    d_timerId( -1 )
{
    ...
    setAxisScale( QwtPlot::yLeft, -2.0, 12.0 );
...
#if 1
//    // Sliding Window with offset of 50% each time
    d_interval = QwtInterval( d_interval.maxValue() - (d_interval.width() * 0.2),
         d_interval.maxValue() + (d_interval.width() * 0.8) );

    //qDebug() << d_interval.minValue() << "," << d_interval.maxValue() << d_interval.width();
    CurveData *data = static_cast<CurveData *>( d_curve->data() );
    ...
    for ( int i = 0; i < QwtScaleDiv::NTickTypes; i++ )
    {
        ...
        for ( int j = 0; j < ticks.size(); j++ )
            ticks[j] += d_interval.width() * 0.8;
        scaleDiv.setTicks( i, ticks );
    }
    setAxisScaleDiv( QwtPlot::xBottom, scaleDiv );

    d_origin->setValue( d_interval.minValue() + d_interval.width(), 0.0 );

    d_paintedPoints = 0;
#else

    d_interval = QwtInterval( d_interval.minValue(), (d_interval.maxValue() + d_interval.width()));
    setAxisScale( QwtPlot::xBottom, d_interval.minValue(), d_interval.maxValue() );
...
void Plot::timerEvent( QTimerEvent *event )
{
    if ( event->timerId() == d_timerId )
    {
        ...
        const double elapsed = d_clock.elapsed() / 250.0;
